<?php
/**
 * Dhl_Intraship_Block_Adminhtml_Sales_Order_Shipment_Create_Intraship_Packages
 *
 * @desc      Parts of this class copied from magento version 1.4.x.x
 ...
 */
class Dhl_Intraship_Block_Adminhtml_Sales_Order_Shipment_Create_Intraship_Packages
    extends Mage_Adminhtml_Block_Widget
{
    /**
     * Construtor
     *
     * @return Dhl_Intraship_Block_Adminhtml_Sales_Order_Shipment_Create_Intraship_Packages
     */
    public function __construct()
    {
        parent::__construct();
        $this->setTemplate(
            'intraship/sales/order/shipment/create/intraship/packages.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        ...
        $this->setChild('add_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label' => Mage::helper('intraship')->__('Add package'),
                    'class' => 'add',
    ...
    public function getDefaultWeight()
    {
        ...
        if (true === $config->useProductWeightAsDefault()) {
            ...
            if ($weight <= 0) {
                $weight = $config->getWeightDefault(
                    $shipment->getShippingAddress()->getCountryId());
            }
        } else {
            $weight = $config->getWeightDefault($shipment->getShippingAddress()
                ->getCountryId());
        }
        return (float) $weight;
    }
